    ...
    def __init__(
        ...
        **kwargs,
        ...
        robot_world_config = RobotWorldConfig.load_from_config(
            ...
            tensor_args=curobo_tensor_args,
    ...
    def setup_costs(self):
        for cost_key in self.args_inference.costs:
            ...
            try:
                ...
                # Inject RobotWorld into cuRobo collision costs
                if cost_key in ("CostTaskSpaceCollisionObjects", "CostTaskSpaceCollisionSelf"):
                    assert (
                        self.robot_world is not None
                    ), f"robot_world must be provided to use {cost_key} with cuRobo SDF"
                    cost = eval(cost_key)(self.planning_task, robot_world=self.robot_world, **cost_options)
                else:
...
class CostTaskSpaceCollisionObjects(CostTaskSpace):
    def __init__(self, planning_task, robot_world, use_only_on_extra_objects=False, **kwargs):
        super().__init__(planning_task, **kwargs)
        # CuRobo RobotWorld is required in this version (no torch_robotics fallback)
        assert robot_world is not None, "robot_world must be provided for CuRobo collision cost"
    ...
    def update_collision_objects_field(self):
        # No-op: using CuRobo RobotWorld only
        return
    ...
    def compute_cost_grad_wrt_cp(
        ...
        **kwargs,
        ...
        # Compute world collision cost and dC/dq from cuRobo to ensure consistency
        env_query_idx = kwargs.get("env_query_idx", None)
        with torch.enable_grad():
            q_for_curobo = q_traj_pos_in_phase.contiguous().requires_grad_(True)
            cost, grad_cost_wrt_q_pos = self.robot_world.get_world_collision_cost_and_grad_wrt_q_from_joint_trajectory(
                q_for_curobo, env_query_idx=env_query_idx
            )
...
class CostTaskSpaceCollisionSelf(CostTaskSpace):
    def __init__(self, planning_task, robot_world, **kwargs):
        super().__init__(planning_task, **kwargs)
        # CuRobo RobotWorld is required in this version (no torch_robotics fallback)
        assert robot_world is not None, "robot_world must be provided for CuRobo self-collision cost"
        self.robot_world = robot_world
    ...
    def compute_cost_grad_wrt_cp(
        ...
        **kwargs,
        ...
        """
        Use cuRobo's kinematics and self-collision to ensure sphere layout matches the
        kernel's expected configuration, avoiding mismatches with MPD sphere sets.
        Returns cost per step and gradient dC/d(control_points).
        """
        with torch.enable_grad():
            # Ensure q is contiguous to avoid .view() issues inside cuRobo and requires grad
            q_for_curobo = q_traj_pos_in_phase.contiguous().requires_grad_(True)
            # Compute cost and dC/dq directly via cuRobo (internally consistent spheres)
            cost, grad_cost_wrt_q_pos = self.robot_world.get_self_collision_cost_and_grad_wrt_q_from_joint_trajectory(
                q_for_curobo
            )

        # Map dC/dq to dC/d(control points)
        grad_cost_wrt_cp = self.compute_grad_cost_wrt_cp(
            control_points,
            grad_cost_wrt_q_pos,
            get_type_single="pos",
